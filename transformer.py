...
class StateTSP(NamedTuple):
    # Fixed input
    loc: torch.Tensor
    time_constraint: torch.Tensor  # 新增时间约束条件
    ...
    time_consumed: torch.Tensor  # Keeps track of time consumed
    ...
    def __getitem__(self, key):
        if torch.is_tensor(key) or isinstance(key, slice):  # If tensor, idx all tensors by this tensor:
            return self._replace(
                ...
                time_consumed=self.time_consumed[key] if self.time_consumed is not None else None,
            )
    ...
    def initialize(loc, visited_dtype=torch.bool):
        ...
        return StateTSP(
            ...
            time_consumed=torch.zeros(batch_size, 1, device=loc.device),  # Initialize time_consumed
            time_constraint=torch.full((batch_size, 1), float('inf'), device=loc.device)  # Initialize time_constraint
        )
    ...
    def update(self, selected, mat, input):
        ...
        # 更新总时间消耗
        new_time = self.time_consumed + mat.get_time(self.prev_a, selected)
        # 检查是否超过约束条件
        feasible = new_time <= self.time_constraint
        # 如果不可行，返回极大值
        if not feasible.all():
            return self._replace(lengths=torch.full_like(self.lengths, float('inf')))
        return self._replace(prev_a=prev_a, visited_=visited_, lengths=lengths, i=self.i + 1, time_consumed=new_time)
...
class AttentionModelFixed(NamedTuple):
    ...
    time_constraint: torch.Tensor  # 新增时间约束字段
    ...
    def __getitem__(self, key):
        if torch.is_tensor(key) or isinstance(key, slice):
            return AttentionModelFixed(
                ...
                time_constraint=self.time_constraint[key]  # 新增时间约束字段
            )
...
class AttentionModel(nn.Module):
    ...
    def _precompute(self, embeddings):
        ...
        # 获取时间约束（假设从输入中获取）
        time_constraint = torch.full((embeddings.size(0), 1), float('inf'), device=embeddings.device)  # 默认无约束
        return AttentionModelFixed(embeddings, fixed_context, *fixed_attention_node_data, time_constraint)
